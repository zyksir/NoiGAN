...
def save_model(model, optimizer, save_variable_list, args, classifier=None, generator=None, best=False):
    ...
    if best:
        torch.save(checkpoint, os.path.join(args.save_path, 'best_checkpoint'))
    else:
...
def main(args):
    ...
    if args.fake:
        # fake_triples = pickle.load(open(os.path.join(args.data_path, "negative%s.pkl" % args.fake), "rb"))
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            if args.do_valid and step % args.valid_steps == 0:
                ...
                if metrics["HITS@10"] > best_hit10:
                    logging.info("best hit@10 now")
                    best_hit10 = metrics["HITS@10"]
                    best_hit10_step = step
                    best_model = kge_model.state_dict()
                    save_variable_list = {
                        'step': step,
                        'current_learning_rate': current_learning_rate,
                        'warm_up_steps': warm_up_steps
